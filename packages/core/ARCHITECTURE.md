...
Derived from `packages/core/src`: **30 subsystems**, **680 files**, **132k lines**, **143 cross-subsystem edges**.
...
| --- | --- | --- | --- | --- | --- | --- |
| `loop` | The drive-to-green engine: turns, tools, gate settling, steering, adapters | core | 168 | 44k | 8 | 22 |
...
| `cli` | Argument parsing, the interactive REPL, and per-mode wiring | core | 22 | 9k | 2 | 18 |
...
| `scaffold` | Stands up a new project from an archetype and configures its gate | optional | 19 | 3k | 3 | 3 |
...
| `agent` ↔ `loop` | `agent/agent-runner.ts:21` → `../loop/loop.types` | `loop/context-hygiene.ts:10` → `../agent/agent.constants` |
...
| `IPlanSchema` | `loop/planning/plan-types.ts:45` | 7 |
| `IGate` | `gate/gate-runner.ts:25` | 21 |
| `IProductPlan` | `loop/planning/plan-types.ts:32` | 9 |
