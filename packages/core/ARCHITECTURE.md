...
Derived from `packages/core/src`: **30 subsystems**, **620 files**, **117k lines**, **140 cross-subsystem edges**.
...
| --- | --- | --- | --- | --- | --- | --- |
| `loop` | The drive-to-green engine: turns, tools, gate settling, steering, adapters | core | 144 | 39k | 8 | 22 |
| `rule-packs` | The ESLint rule packs the gate enforces, grouped by stack | core | 166 | 20k | 3 | 3 |
| `render` | Terminal UI — status bar, menus, wizards, markdown, diffs, spinners | core | 42 | 10k | 7 | 5 |
...
| `agent` | Tool schemas, the model-as-agent wrapper, and the malformed-tool-call repair ladder | core | 10 | 3k | 8 | 9 |
...
| `(root)` | CLI entry, model registry, session persistence — the loose files in src/ | core | 6 | 2k | 5 | 15 |
...
| `gate` | Composes and runs the deterministic gate: linter, stages, tool paths | core | 13 | 2k | 5 | 7 |
| `files` | Reading, creating, and hash-anchored editing of workspace files | core | 9 | 2k | 5 | 1 |
| `lib` | Shared primitives — fs, json, guards, scope globs, SSRF checks, clipboard | core | 18 | 1k | 24 | 0 |
...
| `IConventionProvider` | `loop/conventions-provider.ts:10` | 14 |
