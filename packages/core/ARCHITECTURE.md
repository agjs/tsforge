...
Derived from `packages/core/src`: **30 subsystems**, **546 files**, **97992 lines**, **134 cross-subsystem edges**.
...
flowchart LR
  ...
  stack_detection["stack-detection"]
  agent --> gate
  agent --> inference
  agent --> loop
  agent --> validate
...
| `architecture` ⚠️ | Derives this map from source so the docs cannot drift from the code | optional | 7 | 1034 | 0 | 0 |
