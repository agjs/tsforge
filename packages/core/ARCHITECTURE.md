...
Derived from `packages/core/src`: **30 subsystems**, **574 files**, **104773 lines**, **136 cross-subsystem edges**.
...
| --- | --- | --- | --- | --- | --- | --- |
| `loop` | The drive-to-green engine: turns, tools, gate settling, steering, adapters | core | 110 | 30935 | 7 | 21 |
| `rule-packs` | The ESLint rule packs the gate enforces, grouped by stack | core | 164 | 19000 | 3 | 3 |
| `render` | Terminal UI — status bar, menus, wizards, markdown, diffs, spinners | core | 39 | 8925 | 6 | 5 |
| `cli` | Argument parsing, the interactive REPL, and per-mode wiring | core | 22 | 7279 | 2 | 19 |
...
| `setup` | Onboarding wizard that writes a project's initial tsforge config | optional | 4 | 545 | 2 | 5 |
...
| --- | --- | --- |
| `(root)` ↔ `cli` | `cli.ts:22` → `./cli/worklist-deps` | `cli/config-menu.ts:8` → `../models-config` |
