...
export interface IStatusInfo {
  ...
  /** Live activity indicator (e.g. `⠋ thinking · 12s` / `25m00s`) shown WHILE a
   *  turn runs, so the spinner animates IN the bar instead of on the readline
   *  input line (which it would otherwise clobber). Omitted between turns. */
  activity?: string;
}
