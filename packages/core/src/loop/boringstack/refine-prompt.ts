...
\`\`\`
The explicit \`UseQueryResult<I${feature.id}Item[]>\` + \`Promise<I${feature.id}Item[]>\` anchor the type to YOUR item type (from \`${feature.id}.types.ts\`), so \`data\` is \`I${feature.id}Item[] | undefined\` and \`data ?? []\` is the CORRECT unwrap — NOT an unnecessary condition (the client makes \`data\` optional). NEVER annotate with \`Readable<SuccessResponse<…>>\` and NEVER reach for \`as\`: the domain annotation does all the work. Do NOT check \`error\` (typed \`undefined\`; the \`throwOnError\` middleware throws and React Query catches it).
