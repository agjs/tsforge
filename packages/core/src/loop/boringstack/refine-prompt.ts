...
- **Mutations** (\`${feature.id}.mutations.ts\` — PascalCase file in \`apps/ui/src/features/${camel}/\`): implement real \`useCreate${feature.id}\`, \`useUpdate${feature.id}\`, and \`useDelete${feature.id}\` — each calls \`@/lib/api/client\` (\`apiClient.POST/PATCH/DELETE\`). **The client's \`throwOnError\` middleware THROWS an \`ApiError\` on any non-2xx — it does NOT return an error to check.** So \`error\` is typed \`undefined\`: writing \`if (error) throw error\` is a DEAD \`no-unnecessary-condition\` (error can't be truthy) AND \`only-throw-error\` (you'd be throwing \`undefined\`). React Query catches the thrown ApiError, so invalidate the list query in \`onSuccess\` and surface failures in \`onError\` — never guard \`error\`. Do NOT leave the stub that returns its input. **Match each hook's annotation AND its \`data\` handling to what its route RETURNS** (this is the scaffold's exact idiom — see \`features/accounts/JoinRequests.mutations.ts\`):
  ...
  Same split as the query: \`Type 'Readable<SuccessResponse<...>>' is not assignable\` (openapi-fetch's response wrapper, printed abbreviated) means you have NOT unwrapped — unwrap to match your route's \`response:\` shape (direct value → guard-then-\`return data\`; \`{ data: … }\` envelope → \`return data.data\`, like the scaffold's mutation routes). A plain \`I${feature.id}Item | undefined\` "not assignable to \`I${feature.id}Item\`" = you HAVE unwrapped but \`data\` is optional — that's what the GUARD-then-return above fixes. Never \`as\`-cast it, never "remove the annotation and let TS infer", and never annotate a hook with \`Readable<SuccessResponse<…>>\`.
  (Use the REAL domain fields of ${feature.id} — ${domainFields} — NOT a single placeholder \`name\`.)
- **List query** (\`${feature.id}.queries.ts\` — PascalCase file): implement \`use${feature.id}\` to actually fetch via \`apiClient.GET\` (the scaffold stub returns \`[]\`, so the list is permanently empty and create→appears-in-list is impossible until you do this). **ANNOTATE the hook + queryFn return with your DOMAIN item type** to state the hook's contract, exactly as the scaffold's green hooks do. (\`data\` is typed \`Readable<SuccessResponse<...>>\` — openapi-fetch's wrapper — so \`return data ?? []\` UNWRAPS it to \`I${feature.id}Item[]\`. See the note under the snippet for the two error shapes and how to unwrap per route; never \`as\`, never drop the annotation.) Copy the scaffold's exact shape (see \`features/accounts/JoinRequests.queries.ts\`):
\`\`\`ts
...
\`\`\`
The explicit \`UseQueryResult<I${feature.id}Item[]>\` + \`Promise<I${feature.id}Item[]>\` state the hook's contract and match the scaffold's green shape. \`data\` from \`apiClient.GET\` is typed \`Readable<SuccessResponse<...>>\` — openapi-fetch's response WRAPPER, not your item type — so you must UNWRAP it. **Two DIFFERENT errors, two DIFFERENT fixes — do not confuse them:** (a) \`Type 'Readable<SuccessResponse<...>>' is not assignable to type 'I${feature.id}Item[]'\` (tsc prints the abbreviated \`<...>\`) means you have NOT unwrapped the wrapper. Unwrap it to match your route's \`response:\` shape — a route that returns the value directly gives \`return data ?? []\` (list) / guard-then-\`return data\` (one item); a route whose schema wraps \`{ data: … }\` gives \`return data.data\` (guard \`if (!data?.data) throw …\`). Do NOT \`as\`-cast it and do NOT "remove the annotation and let TS infer" — neither is the scaffold's idiom. (b) "\`I${feature.id}Item[] | undefined\` is not assignable to \`I${feature.id}Item[]\`" means you HAVE unwrapped but \`data\` is optional — add \`?? []\` (list) or the guard (one item); do NOT touch the schema for this one. If after unwrapping \`data.data\` is typed \`any\`/\`unknown\` or \`.data\` is missing, your generated api types are stale for a \`response:\` schema you changed — make the route's \`response:\` schema correct and the gate re-runs \`generate:api\` next cycle. Do NOT check \`error\` (typed \`undefined\`; the \`throwOnError\` middleware throws and React Query catches it).
