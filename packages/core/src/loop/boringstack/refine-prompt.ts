...
\`\`\`
then in the component body \`const editHandler = makeIdHandler(onEdit);\` (and \`deleteHandler\`), and in JSX \`onClick={editHandler(row.id)}\`. The prop is a call expression, not a literal arrow, so the rule passes. This curried-helper pattern is the scaffold's own gate-green convention (see JoinRequestsPage example). Do NOT call \`useCallback\` in the component body to stabilise the handler. react-component-architecture forbids React's STATE-PRIMITIVE hooks (\`useState\`/\`useReducer\`/\`useEffect\`/\`useLayoutEffect\`/\`useMemo\`/\`useCallback\`/\`useRef\`) from being called DIRECTLY in a component that returns JSX — those must live inside a custom hook in a co-located \`<Component>.hooks.ts\` (else "must be in a custom hook (.hooks.ts), not in component body"). The body MAY still call other hooks inline: your own custom hook(s) (e.g. \`use${feature.id}Page()\`), \`useTranslation()\`, and \`useId\`/\`useTransition\`/\`useDeferredValue\` are all allowed there — the scaffold's JoinRequestsPage body itself calls both \`useTranslation()\` and \`useJoinRequestsPage()\`. The \`makeIdHandler\` call-expression helper needs NO hook at all, so it's the simplest fix for a row handler — use it rather than adding a \`useCallback\`. **For a form/submit or any zero-arg handler that runs LOGIC (validate, call a mutation, reset), define it INSIDE the \`<Component>.hooks.ts\` custom hook and return it, then reference \`view.onSubmit\` in JSX — exactly the scaffold's green form pattern (\`features/auth/components/LoginPage/LoginPage.tsx\`: \`const view = use${feature.id}Page(props);\` then \`<form onSubmit={view.onSubmit}>\`).** A bare \`view.onSubmit\` member reference is not an arrow/function-expression, so \`jsx-no-bind\` passes; and because the handler lives in the hook, the state primitives it needs (\`useState\`/\`useCallback\`/etc.) are legal there. Do NOT define it as a body-local \`const handleSubmit = () => { … }\` in the \`.tsx\` — that both trips the component-architecture rules and is not the scaffold idiom. (A handler ALREADY handed in as a prop — e.g. an \`onEdit\` the parent passed — is fine to reference bare as \`onClick={props.onEdit}\`; the rule is only that YOU must not DEFINE a handler inline in the component body — put it in the hook and reference \`view.onSubmit\`.)
