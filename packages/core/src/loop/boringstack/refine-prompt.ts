...
\`\`\`
then in the component body \`const editHandler = makeIdHandler(onEdit);\` (and \`deleteHandler\`), and in JSX \`onClick={editHandler(row.id)}\`. The prop is a call expression, not a literal arrow, so the rule passes. This curried-helper pattern is the scaffold's own gate-green convention (see JoinRequestsPage example). Do NOT reach for \`useCallback\` in the component body to stabilise the handler: react-component-architecture requires React's PRIMITIVE hooks (\`useState\`/\`useCallback\`/\`useMemo\`/\`useEffect\`/\`useRef\`/…) to be called INSIDE a custom hook that lives in a co-located \`<Component>.hooks.ts\` — the component body may only CALL that one custom hook (e.g. \`const { editHandler } = use${feature.id}Form();\`), it must never call \`useCallback\` (or another primitive hook) DIRECTLY in the body (else "Hook 'useCallback' must be in a custom hook, not in component body"). The \`makeIdHandler\` call-expression helper needs NO hook at all, so it's the simplest fix — use it rather than introducing a custom hook just to memoise a row handler. For a zero-arg handler pass a named function reference (\`onClick={handleSubmit}\`), never an inline arrow.
