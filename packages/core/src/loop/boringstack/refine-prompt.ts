...
- **List query** (\`${feature.id}.queries.ts\` — PascalCase file): implement \`use${feature.id}\` to actually fetch via \`apiClient.GET\` (the scaffold stub returns \`[]\`, so the list is permanently empty and create→appears-in-list is impossible until you do this). **ANNOTATE the hook + queryFn return with your DOMAIN item type** to state the hook's contract, exactly as the scaffold's green hooks do. (The near-green cluster has separate causes — see the note under the snippet: \`Readable<SuccessResponse>\` is killed by the API route's \`response:\` schema; \`… | undefined is not assignable\` means \`data\` is optional and you haven't unwrapped — add \`return data ?? []\`; \`Unnecessary ??\` is the OPPOSITE — \`data\` is already non-optional there, so the \`?? []\` is redundant and must be REMOVED, not added. The annotation alone kills none of these, because it does not retype the destructured \`data\`; and NEVER \`as\`.) Copy the scaffold's exact shape (see \`features/accounts/JoinRequests.queries.ts\`):
\`\`\`ts
