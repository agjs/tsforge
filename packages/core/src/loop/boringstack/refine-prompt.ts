...
- **Mutations** (\`${feature.id}.mutations.ts\` — PascalCase file in \`apps/ui/src/features/${camel}/\`): implement real \`useCreate${feature.id}\`, \`useUpdate${feature.id}\`, and \`useDelete${feature.id}\` — each calls \`@/lib/api/client\` (\`apiClient.POST/PATCH/DELETE\`). **The client's \`throwOnError\` middleware THROWS an \`ApiError\` on any non-2xx — it does NOT return an error to check.** So \`error\` is typed \`undefined\`: writing \`if (error) throw error\` is a DEAD \`no-unnecessary-condition\` (error can't be truthy) AND \`only-throw-error\` (you'd be throwing \`undefined\`). Just read \`data\`: \`const { data } = await apiClient.POST(…); return data;\`. React Query catches the thrown ApiError, so invalidate the list query in \`onSuccess\` and surface failures in \`onError\` — never guard \`error\`. Do NOT leave the stub that returns its input. **ANNOTATE each mutation hook with your DOMAIN type** (same contract-stating shape as the query, matching the scaffold): \`export function useCreate${feature.id}(): UseMutationResult<I${feature.id}Item, unknown, ${feature.id}CreateInput> { … }\` — 1st generic = the returned item type, 2nd = \`unknown\` (the error; matches the scaffold), 3rd = the input variables type. As with the query, what TYPES \`data\` from \`apiClient.POST/PATCH\` is the API route's \`response:\` schema (via \`generate:api\`), not this annotation — if \`data\` comes back \`Readable<SuccessResponse<…>>\` / not-assignable, add the missing \`response:\` schema to the mutation's route, never \`as\`-cast it. Never annotate a hook with \`Readable<SuccessResponse<…>>\`.
...
\`\`\`
The explicit \`UseQueryResult<I${feature.id}Item[]>\` + \`Promise<I${feature.id}Item[]>\` state the hook's contract and match the scaffold's green shape. But what actually TYPES \`data\` is the API route's \`response:\` schema: the gate re-runs \`generate:api\` every cycle from the live API's \`/swagger/json\`, so once your \`${camel}.routes.ts\` list route declares \`response: t.Array(<ItemSchema>)\`, \`data\` is generated as \`I${feature.id}Item[] | undefined\` and \`data ?? []\` is the CORRECT unwrap (the client makes \`data\` optional). If instead you see \`Readable<SuccessResponse<…>>\` or "\`data\` … is not assignable", the route is MISSING its \`response:\` schema (or isn't mounted) — fix THAT on the API side; a hook/fn return annotation only checks the returned expression, it does NOT retype \`const { data } = await apiClient.GET(…)\`. NEVER annotate with \`Readable<SuccessResponse<…>>\` and NEVER reach for \`as\` to force it. Do NOT check \`error\` (typed \`undefined\`; the \`throwOnError\` middleware throws and React Query catches it).
