...
\`\`\`
then in the component body \`const editHandler = makeIdHandler(onEdit);\` (and \`deleteHandler\`), and in JSX \`onClick={editHandler(row.id)}\`. The prop is a call expression, not a literal arrow, so the rule passes. This curried-helper pattern is the scaffold's own gate-green convention (see JoinRequestsPage example). Do NOT reach for \`useCallback\` to stabilise the handler: a hook (\`useCallback\`/\`useMemo\`/any \`use*\`) may NEVER be called in a component body — react-component-architecture requires every hook to live in a co-located \`<Component>.hooks.ts\` custom hook (else "Hook 'useCallback' must be in a custom hook, not in component body"). The \`makeIdHandler\` call-expression helper needs no hook at all, so it sidesteps that rule entirely — use it, not a body \`useCallback\`. For a zero-arg handler pass a named function reference (\`onClick={handleSubmit}\`), never an inline arrow.
