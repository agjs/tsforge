...
- **List query** (\`${feature.id}.queries.ts\` — PascalCase file): implement \`use${feature.id}\` to actually fetch via \`apiClient.GET\` (the scaffold stub returns \`[]\`, so the list is permanently empty and create→appears-in-list is impossible until you do this). **ANNOTATE the hook + queryFn return with your DOMAIN item type** to state the hook's contract, exactly as the scaffold's green hooks do. (What actually KILLS the near-green \`Readable<SuccessResponse>\` / \`no-any-assertion\` (\`as\`) / \`Unnecessary ??\` / \`… is not assignable\` cluster is the API route's \`response:\` schema — see the note under the snippet; the annotation alone does not, because it does not retype the destructured \`data\`.) Copy the scaffold's exact shape (see \`features/accounts/JoinRequests.queries.ts\`):
\`\`\`ts
