...
- **Mutations** (\`${feature.id}.mutations.ts\` — PascalCase file in \`apps/ui/src/features/${camel}/\`): implement real \`useCreate${feature.id}\`, \`useUpdate${feature.id}\`, and \`useDelete${feature.id}\` — each calls \`@/lib/api/client\` (\`apiClient.POST/PATCH/DELETE\`). **The client's \`throwOnError\` middleware THROWS an \`ApiError\` on any non-2xx — it does NOT return an error to check.** So \`error\` is typed \`undefined\`: writing \`if (error) throw error\` is a DEAD \`no-unnecessary-condition\` (error can't be truthy) AND \`only-throw-error\` (you'd be throwing \`undefined\`). React Query catches the thrown ApiError, so invalidate the list query in \`onSuccess\` and surface failures in \`onError\` — never guard \`error\`. Do NOT leave the stub that returns its input. **Match each hook's annotation AND its \`data\` handling to what its route RETURNS** (this is the scaffold's exact idiom — see \`features/accounts/JoinRequests.mutations.ts\`):
  ...
  What TYPES \`data\` is the route's \`response:\` schema (via \`generate:api\`), not the annotation. Same two-errors split as the query: \`Readable<SuccessResponse<…>>\` = add the missing \`response:\` schema to that mutation's route (API-side fix); a plain \`I${feature.id}Item | undefined\` "not assignable to \`I${feature.id}Item\`" = the schema is fine, \`data\` is just optional — that's what the GUARD-then-return above fixes (do NOT rewrite an already-correct schema). Never \`as\`-cast it and never annotate a hook with \`Readable<SuccessResponse<…>>\`.
  (Use the REAL domain fields of ${feature.id} — ${domainFields} — NOT a single placeholder \`name\`.)
- **List query** (\`${feature.id}.queries.ts\` — PascalCase file): implement \`use${feature.id}\` to actually fetch via \`apiClient.GET\` (the scaffold stub returns \`[]\`, so the list is permanently empty and create→appears-in-list is impossible until you do this). **ANNOTATE the hook + queryFn return with your DOMAIN item type** to state the hook's contract, exactly as the scaffold's green hooks do. (The near-green cluster has two separate causes — see the note under the snippet: \`Readable<SuccessResponse<{ 200: {} }>>\` (an EMPTY inner body) means the API route has no \`response:\` schema — fix it there; \`… | undefined is not assignable\` means the schema is fine and \`data\` is just optional — unwrap on the consumer with \`return data ?? []\`. The annotation alone kills neither, because it does not retype the destructured \`data\`; and NEVER \`as\`. Follow the compiler on the unwrap: if it instead flags \`Unnecessary ??\`, \`data\` is already non-nullable at that spot — drop the redundant \`?? []\`.) Copy the scaffold's exact shape (see \`features/accounts/JoinRequests.queries.ts\`):
\`\`\`ts
...
\`\`\`
The explicit \`UseQueryResult<I${feature.id}Item[]>\` + \`Promise<I${feature.id}Item[]>\` state the hook's contract and match the scaffold's green shape. But what actually TYPES \`data\` is the API route's \`response:\` schema: the gate re-runs \`generate:api\` every cycle from the live API's \`/swagger/json\`, so once your \`${camel}.routes.ts\` list route declares \`response: t.Array(<ItemSchema>)\`, \`data\` is generated as \`I${feature.id}Item[] | undefined\` and \`data ?? []\` is the CORRECT unwrap (the client makes \`data\` optional). **Two DIFFERENT errors, two DIFFERENT fixes — do not confuse them:** (a) \`Readable<SuccessResponse<{ 200: {} }>>\` — note the EMPTY inner \`{ 200: {} }\` — means the route's 200 has no \`response:\` schema (\`Readable<SuccessResponse<…>>\` is openapi-fetch's NORMAL wrapper; only the empty inner is the tell), so fix it on the API side; a hook/fn return annotation only checks the returned expression, it does NOT retype \`const { data } = await apiClient.GET(…)\`. (b) "\`I${feature.id}Item[] | undefined\` is not assignable to \`I${feature.id}Item[]\`" means the schema is ALREADY correct and \`data\` is just optional — the fix is the CONSUMER unwrap (\`return data ?? []\`), NOT touching the schema (rewriting an already-correct schema is a classic near-green oscillation). NEVER annotate with \`Readable<SuccessResponse<…>>\` and NEVER reach for \`as\` to force either. Do NOT check \`error\` (typed \`undefined\`; the \`throwOnError\` middleware throws and React Query catches it).
