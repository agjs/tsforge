...
function structuredSteerMessage(message: string, file: string): string {
  ...
  // on `Readable<SuccessResponse` broadly. `Readable<SuccessResponse<...>>` is openapi-fetch's response
  // wrapper (unions the media types); the fix is to UNWRAP it to your domain type exactly as the
  // scaffold's green hooks do — the shape of the unwrap depends on what YOUR route's `response:` schema
  // returns (a bare value vs an `{ data: … }` envelope). Never cast, never drop the annotation.
  if (message.includes("Readable<SuccessResponse")) {
    return `${message}\n↳ \`Readable<SuccessResponse<...>>\` is openapi-fetch's response wrapper (it unions the response media types); it is not directly assignable to your domain type. UNWRAP it to match your hook's annotation, exactly like the scaffold's green hooks — the unwrap depends on what YOUR route's \`response:\` schema returns: (a) a route that returns the value DIRECTLY (\`response: t.Array(<Item>)\` / \`response: <Item>\`) → \`const { data } = await apiClient.GET(…); return data ?? []\` for a list, or guard-then-return for one item (\`if (!data) throw new ApiError(0, { message: "Empty response" }); return data;\`); (b) a route whose schema WRAPS the value in an envelope (\`response: t.Object({ data: <Item> })\`, like the scaffold's mutation routes) → read the inner: \`if (!data?.data) throw new ApiError(0, { message: "Empty response" }); return data.data;\` (see \`JoinRequests.mutations.ts\`). Pick the ONE that matches your \`response:\` schema — do NOT blindly add \`.data\`. Keep the hook annotations (\`UseQueryResult<IXItem[]>\` / \`UseMutationResult<IXItem, unknown, Vars>\`; DELETE → \`UseMutationResult<void, unknown, string>\`). NEVER \`as\`-cast the \`Readable\` away and NEVER "remove the annotation and let TS infer" (both are dead ends the scaffold does not use). If \`data.data\` is typed \`any\`/\`unknown\` or \`.data\` is missing, your generated api types are stale for a \`response:\` schema you just changed — the gate re-runs \`generate:api\` next cycle from /swagger/json, so make the route's \`response:\` schema correct and let it regenerate.`;
  }
