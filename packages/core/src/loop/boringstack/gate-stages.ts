...
function structuredSteerMessage(message: string, file: string): string {
  ...
  // fields — see openapi-fetch's `data: ParseAsResponse<Readable<SuccessResponse<…>>>`). It only
  // surfaces as a raw, non-assignable type when the INNER response body is EMPTY/untyped
  // (`SuccessResponse<{ 200: {} }>`) — i.e. the route has no explicit `response:` TypeBox schema for
  // its 200. So the fix for THAT error is the API `response:` schema; with it, the wrapper resolves
  // to the domain type and `data` (still optional) is unwrapped on the consumer.
  if (message.includes("Readable<SuccessResponse")) {
    return `${message}\n↳ \`Readable<SuccessResponse<…>>\` is openapi-fetch's NORMAL response wrapper (it strips writeOnly fields; every response's \`data\` is typed \`Readable<SuccessResponse<…>>\`). It only shows up as a RAW, non-assignable type when the INNER body is EMPTY — e.g. \`SuccessResponse<{ 200: {} }>\` — which means THIS route has no explicit \`response:\` TypeBox schema for its 200 (so \`generate:api\`, which the gate re-runs every cycle from /swagger/json, has nothing to type the body with). FIX AT THE API SOURCE: on \`apps/api/src/api/<resource>/<resource>.routes.ts\` add an explicit \`response:\` schema — list → \`response: t.Array(<ItemSchema>)\`, get-one/create/update → \`response: <ItemSchema>\` (covers the \`.mutations.ts\` POST/PATCH case), delete → \`response: t.Null()\` — matching EXACTLY what the service returns (a nullable column is \`t.Optional(t.Union([t.String(), t.Null()]))\`, not \`t.Optional(t.String())\`); make sure the route is mounted (shows in /swagger/json). Once the schema exists the wrapper resolves to your domain type. Do NOT try to fix this by only re-typing the consumer — a hook/fn return annotation checks the RETURNED expression, it does NOT retype \`const { data } = await apiClient.GET(…)\` — and NEVER \`as\`-cast the \`Readable\` away. THEN unwrap \`data\` (still typed optional) by ROUTE KIND, per the scaffold: a LIST query is \`UseQueryResult<IXItem[]>\` + \`return data ?? []\` (\`JoinRequests.queries.ts\`); a CREATE/UPDATE mutation is \`UseMutationResult<IXItem, unknown, Vars>\` and GUARDS then returns the item (\`const { data } = await apiClient.POST(…); if (!data) throw new ApiError(0, { message: "Empty response" }); return data;\`, \`JoinRequests.mutations.ts\`) — NOT \`?? []\`; a DELETE is \`UseMutationResult<void, unknown, string>\` → \`await apiClient.DELETE(…)\` returning nothing.`;
  }
