...
function structuredSteerMessage(message: string, file: string): string {
  ...
  if (message.includes("Readable<SuccessResponse")) {
    return `${message}\n↳ \`Readable<SuccessResponse<…>>\` is the api-client's NORMAL, UNIVERSAL response type — Elysia's swagger emits three media types (json/multipart/text) for EVERY route (the scaffold's own auth/dashboard routes are identical), so you CANNOT remove it by editing the route or the \`response:\` schema — do NOT try, and NEVER \`as\`-cast it away. Fix it by ANNOTATING the hook + fn with YOUR DOMAIN type, exactly as the scaffold's green hooks do (\`features/accounts/JoinRequests.queries.ts\`): a query is \`export function useX(): UseQueryResult<IXItem[]> { return useQuery({ queryKey: X_QUERY_KEYS.list, queryFn: async (): Promise<IXItem[]> => { const { data } = await apiClient.GET(…); return data ?? []; } }); }\`; a mutation is \`export function useCreateX(): UseMutationResult<IXItem, unknown, XCreateInput> { return useMutation({ mutationFn: async (input): Promise<IXItem> => { const { data } = await apiClient.POST(…, { body: input }); return data; } }); }\`. The DOMAIN annotation on the hook (\`UseQueryResult<IXItem[]>\` / \`UseMutationResult<IXItem, unknown, Vars>\`) AND the fn (\`Promise<IXItem[]>\`) anchors the type so \`Readable\` never reaches the consumer — do NOT remove the annotation (an inferred hook re-surfaces \`Readable\` at the consumer) and do NOT annotate with \`Readable\` itself. Unwrap for THIS route's shape: a list/object route returns the value directly (\`return data ?? []\` / \`return data\`); a route that wraps \`{ data: … }\` (some scaffold routes) reads \`return data.data\` — match your \`response:\` schema, don't blindly add \`.data\`.`;
  }
