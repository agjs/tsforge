...
function structuredSteerMessage(message: string, file: string): string {
  ...
  // `Readable<SuccessResponse<…>>` is openapi-fetch's response-wrapper type — normal on its own. The
  // near-green error we can diagnose precisely is the EMPTY-inner form (`SuccessResponse<{ <code>: {} }>`):
  // the route's success response has no `response:` TypeBox schema, so `generate:api` has nothing to
  // type the body with. We MATCH only that empty-inner signature so the "add the response schema"
  // steer is always correct for what it fires on; a non-empty Readable (rarer) falls through to the
  // front-loaded guide rather than getting mis-steered to touch an already-correct schema.
  if (/Readable<SuccessResponse<\{\s*\d+:\s*\{\s*\}\s*\}\s*>/u.test(message)) {
    return `${message}\n↳ This \`Readable<SuccessResponse<{ <code>: {} }>>\` has an EMPTY inner response body — THIS route has no explicit \`response:\` TypeBox schema for that status, so \`generate:api\` (re-run every gate cycle from /swagger/json) can't type the body. FIX AT THE API SOURCE on \`apps/api/src/api/<resource>/<resource>.routes.ts\`: add the \`response:\` schema — list → \`t.Array(<ItemSchema>)\`, get-one/create/update → \`<ItemSchema>\` (covers the \`.mutations.ts\` POST/PATCH case), delete → \`t.Null()\` — matching EXACTLY what the service returns (a nullable column is \`t.Optional(t.Union([t.String(), t.Null()]))\`, not \`t.Optional(t.String())\`); ensure the route is mounted (shows in /swagger/json). NEVER \`as\`-cast the \`Readable\` away, and do NOT expect a consumer return annotation to fix it — an annotation checks the RETURNED expression, it does NOT retype \`const { data } = await apiClient.GET(…)\`. Once the schema exists, unwrap \`data\` (typed optional) by ROUTE KIND, exactly like the scaffold: a LIST query is \`UseQueryResult<IXItem[]>\` + \`return data ?? []\` (\`JoinRequests.queries.ts\`); a CREATE/UPDATE mutation is \`UseMutationResult<IXItem, unknown, Vars>\` and GUARDS then returns the item (\`const { data } = await apiClient.POST(…); if (!data) throw new ApiError(0, { message: "Empty response" }); return data;\`, \`JoinRequests.mutations.ts\`) — NOT \`?? []\`; a DELETE is \`UseMutationResult<void, unknown, string>\` → \`await apiClient.DELETE(…)\` returning nothing. If your route ALREADY has a matching \`response:\` schema and the error persists, diff your hook against that scaffold example rather than adding a second schema.`;
  }
