...
export function signatureToError(sig: string): IErrorItem {
  ...
  if (structured !== null) {
    ...
    // Two syntax-error shapes need OPPOSITE guidance, and both keep their located
    // signature (nothing is deleted — deleting gate diagnostics is gate-relaxing):
    //
    //   1. A REAL break (`Parsing error: … expected`) breaks the type-aware program for
    //      the whole app. Fix THIS file first and REWRITE it in full — a surgical patch
    //      on an already-broken file usually re-breaks its braces/generics (the thrash).
    //
    //   2. A `parserOptions.project` / "ESLint was configured to run on …" error is NOT a
    //      break: it's what EVERY other file reports once the real break above kills the
    //      program. It reads like a per-file tsconfig miss, so the model chases each one.
    //      Steer it to the decision it can't make from one signature: if any sibling has a
    //      real break, this is downstream noise that clears when that break is fixed — do
    //      NOT edit this file; otherwise the file genuinely isn't in tsconfig (add it).
    const isParserProjectError =
      rule === "syntax" &&
    ...
    const steer = isRealSyntaxBreak
      ...
      : isParserProjectError
        ? `${message} — this is almost always DOWNSTREAM noise: a real \`Parsing error: … expected\` in ANOTHER file in this app kills the type-aware program, and every remaining file then reports this. Find and REWRITE that one broken file — this and the other parserOptions errors clear together. Do NOT edit this file to chase it. Only if NO file has a real break is this a genuine tsconfig miss (add the file to the tsconfig \`include\`).`
        : message;
