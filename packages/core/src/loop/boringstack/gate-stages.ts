...
function structuredSteerMessage(message: string, file: string): string {
  ...
  // `Readable<SuccessResponse<…>>` is openapi-fetch's NORMAL response wrapper (it strips writeOnly
  // fields — see openapi-fetch's `data: ParseAsResponse<Readable<SuccessResponse<…>>>`). It surfaces
  // as a raw, non-assignable type for two reasons, told apart by the INNER: EMPTY inner
  // (`SuccessResponse<{ 200: {} }>`) = the route has no `response:` schema (add it); a domain inner
  // that still won't assign = the schema exists but its shape differs from the target (writeOnly /
  // domain mismatch) — reconcile them. Both are API/type-source fixes, never a consumer `as`.
  if (message.includes("Readable<SuccessResponse")) {
    return `${message}\n↳ \`Readable<SuccessResponse<…>>\` is openapi-fetch's NORMAL response wrapper (it strips writeOnly fields; every response's \`data\` is typed \`Readable<SuccessResponse<…>>\`). It becomes a RAW, non-assignable type for one of two reasons — READ THE INNER: (1) if the inner is EMPTY — \`SuccessResponse<{ 200: {} }>\` — THIS route has no explicit \`response:\` TypeBox schema for its 200 (so \`generate:api\`, re-run every cycle from /swagger/json, has nothing to type the body with): ADD the schema. (2) if the inner IS a domain shape but still not assignable to your hook's type, the schema EXISTS but its shape differs from your target — reconcile them (a writeOnly field openapi-fetch strips from the response, or a field the \`response:\` schema types differently than \`I<Item>\`): make the \`response:\` schema and the consumer type agree; do NOT add a second schema. Either way FIX AT THE API/type SOURCE — on \`apps/api/src/api/<resource>/<resource>.routes.ts\` the \`response:\` schema is list → \`t.Array(<ItemSchema>)\`, get-one/create/update → \`<ItemSchema>\` (covers the \`.mutations.ts\` POST/PATCH case), delete → \`t.Null()\` — matching EXACTLY what the service returns (a nullable column is \`t.Optional(t.Union([t.String(), t.Null()]))\`, not \`t.Optional(t.String())\`); make sure the route is mounted (shows in /swagger/json). Once schema and consumer type agree the wrapper resolves to your domain type. Do NOT try to fix this by only re-typing the consumer — a hook/fn return annotation checks the RETURNED expression, it does NOT retype \`const { data } = await apiClient.GET(…)\` — and NEVER \`as\`-cast the \`Readable\` away. THEN unwrap \`data\` (still typed optional) by ROUTE KIND, per the scaffold: a LIST query is \`UseQueryResult<IXItem[]>\` + \`return data ?? []\` (\`JoinRequests.queries.ts\`); a CREATE/UPDATE mutation is \`UseMutationResult<IXItem, unknown, Vars>\` and GUARDS then returns the item (\`const { data } = await apiClient.POST(…); if (!data) throw new ApiError(0, { message: "Empty response" }); return data;\`, \`JoinRequests.mutations.ts\`) — NOT \`?? []\`; a DELETE is \`UseMutationResult<void, unknown, string>\` → \`await apiClient.DELETE(…)\` returning nothing.`;
  }
