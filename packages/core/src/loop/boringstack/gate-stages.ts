...
function structuredSteerMessage(message: string, file: string): string {
  ...
  // `Readable<SuccessResponse<…>>` is openapi-fetch's UNTYPED-BODY FALLBACK: generate:api typed
  // this route's response body as a stream because the API route has no explicit `response:` TypeBox
  // schema (or the route isn't mounted, so it's absent from /swagger/json). It is NOT the normal type —
  // the scaffold's green routes all declare `response:` and their generated types never contain
  // `Readable`. So the FIX IS AT THE API SOURCE (the response schema), NOT a consumer annotation.
  if (message.includes("Readable<SuccessResponse")) {
    return `${message}\n↳ \`Readable<SuccessResponse<…>>\` is NOT the normal api-client type — it is openapi-fetch's UNTYPED-BODY FALLBACK. It appears because \`generate:api\` (which the gate re-runs every cycle from the LIVE API's /swagger/json) typed THIS route's response body as an untyped stream — because the Elysia route has no explicit \`response:\` TypeBox schema, or the route isn't mounted so it's absent from the spec. The scaffold's green routes all declare \`response:\` and their generated types never contain \`Readable\`. FIX IT AT THE API SOURCE, not the consumer: on \`apps/api/src/api/<resource>/<resource>.routes.ts\` add an explicit \`response:\` schema to this route — list → \`response: t.Array(<ItemSchema>)\`, get-one/create/update → \`response: <ItemSchema>\` (the single affected item; this covers the \`.mutations.ts\` POST/PATCH case), delete → \`response: t.Null()\` — matching EXACTLY what the service returns (a nullable column is \`t.Optional(t.Union([t.String(), t.Null()]))\`, not \`t.Optional(t.String())\`), and make sure the route is mounted (it must show in /swagger/json). The gate then re-runs generate:api and \`data\` becomes your domain type. NEVER \`as\`-cast the \`Readable\` away, and do NOT try to fix assignability by only re-typing the consumer — a return-type annotation on the hook/fn checks the RETURNED expression, it does NOT retype \`const { data } = await apiClient.GET(…)\`; if \`data\` is \`Readable\`, only the API response schema fixes it. Once \`data\` is typed, the consumer unwrap DEPENDS ON THE ROUTE KIND (\`data\` is always optional, so it needs unwrapping): a LIST query is \`UseQueryResult<IXItem[]>\` + \`return data ?? []\` (per \`JoinRequests.queries.ts\`); a CREATE/UPDATE mutation is \`UseMutationResult<IXItem, unknown, Vars>\` and GUARDS then returns the item (\`const { data } = await apiClient.POST(…); if (!data) throw new ApiError(0, { message: "Empty response" }); return data;\`, per \`JoinRequests.mutations.ts\`) — NOT \`?? []\`; a DELETE mutation is \`UseMutationResult<void, unknown, string>\` and just \`await apiClient.DELETE(…)\` returning nothing. A plain \`IXItem | undefined\` "not assignable" WITHOUT \`Readable\` means the schema is already fine — you only need that unwrap, do NOT rewrite a correct schema. — in short, the API \`response:\` schema makes \`data\` TYPED (kills \`Readable\`); the route-kind unwrap above makes that typed-but-optional \`data\` ASSIGNABLE to the hook's contract.`;
  }

  // An `X | undefined is not assignable to X` with NO `Readable` is the OPTIONAL-data case: the
  // response schema is already correct (data is typed), the value is just optional and unwrapped
  // wrong. The fix is the CONSUMER unwrap — NOT re-touching the schema (that reopens near-green
  // schema-thrash). Matched narrowly (`| undefined` + `not assignable`) so it never fires on the
  // Readable case (returned above) nor a generic non-optional mismatch.
  if (message.includes("| undefined") && message.includes("not assignable")) {
    return `${message}\n↳ An \`… | undefined is not assignable to …\` (no \`Readable\`) means a value typed OPTIONAL is used where a non-optional is required. MOST COMMONLY this is the api-client \`data\` at a query/mutation consumer — the client ALWAYS types \`data\` optional, and the \`response:\` schema is ALREADY correct, so do NOT rewrite the schema (that is a classic near-green oscillation). UNWRAP it by ROUTE KIND: a LIST query returns \`return data ?? []\`; a CREATE/UPDATE mutation GUARDS then returns the item (\`const { data } = await apiClient.POST(…); if (!data) throw new ApiError(0, { message: "Empty response" }); return data;\`); a DELETE mutation is \`UseMutationResult<void, unknown, string>\` → \`await apiClient.DELETE(…)\` and returns nothing. NEVER \`as\`-cast it. (If this is instead an API route where a NULLABLE Drizzle column — \`string | null\` — is checked against \`t.Optional(t.String())\` — \`string | undefined\` — widen the \`response:\` field to \`t.Optional(t.Union([t.String(), t.Null()]))\`: \`null\` is not \`undefined\`.)`;
  }
