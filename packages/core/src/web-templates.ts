...
const REACT_GUIDANCE = [
  ...
  "CONVENTIONS (follow them — this is judged on structure, not just compiling):",
  "  • Routing: file-based via TanStack Router. Add a route by creating",
  "    src/routes/<name>.tsx that exports `Route = createFileRoute('/<name>')({...})`.",
  "    routeTree.gen.ts is GENERATED by the build — never edit it.",
  "  • UI: use shadcn/ui components from @/components/ui (Button exists as an",
  "    example). Add more shadcn-style components there following the same pattern",
  "    (cva + cn() + Tailwind tokens like bg-primary/text-muted-foreground).",
  "  • Compose app components in src/components/*.tsx — one per file, small.",
  "  • Data/server state: use TanStack Query (useQuery/useMutation); a",
  "    QueryClientProvider is already wired in src/main.tsx.",
  "  • State/logic: hooks in src/hooks/*.ts; shared types in src/types.ts.",
  "  • Style with Tailwind classes via className. Use the theme tokens",
  "    (bg-background, text-foreground, border-border, etc.), not raw colors.",
