...
test("stand-down LATCHES across a within-band count wobble (1→2) — not dropped", () => {
  const s = freshState();

  // Arm rotation on a count-1 plateau (3 distinct sigs).
  trackNearGreenRotation(s, 1, [e("a", "x.ts")]);
  trackNearGreenRotation(s, 1, [e("b", "y.ts")]);
  trackNearGreenRotation(s, 1, [e("c", "z.ts")]);
  expect(s.nearGreenRotation).toBe(true);

  // A wobble WITHIN the near-green band: count 2 (still ≤ N=2) with a 2-error set. This breaks the
  // count plateau for one cycle — but the model is still mid-rotation (about to do the atomic
  // completion), so the latch must HOLD. If it dropped, WS-B would revert the very next spray.
  trackNearGreenRotation(s, 2, [e("d", "w.ts"), e("e", "v.ts")]);
  expect(s.nearGreenRotation).toBe(true);
});

test("stand-down DISARMS when the frontier stabilizes on a single stuck error", () => {
  const s = freshState();
  ...
  // The frontier settles on ONE error for a full window → rotation is over; hand it to the
  // escalation ladder (WS-B re-engages). The latch clears.
  trackNearGreenRotation(s, 1, [e("stuck", "p.ts")]);
  trackNearGreenRotation(s, 1, [e("stuck", "p.ts")]);
  trackNearGreenRotation(s, 1, [e("stuck", "p.ts")]);
  expect(s.nearGreenRotation).toBe(false);
});
