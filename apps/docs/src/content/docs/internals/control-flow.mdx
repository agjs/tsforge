...
and `buildTsService()` starts the language service. Then it hands off to `runMainLoop()`.
...
`trackErrorAges()` feed `checkStuck()`, which separates *slow* from *stalled*. An error
count that keeps falling is progress even if it is slow. The same error set turn after
turn is not.
...
| R3 | Narrow the change to one file, one error |
