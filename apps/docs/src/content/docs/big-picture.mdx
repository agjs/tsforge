...
The idea was first tested against a 27B local model; today the default local stack is **DeepSeek‑V4‑Flash served on-box via a custom vLLM setup**. The same loop works with larger or hosted models — they usually need fewer retries, and they still benefit from an external oracle and stack-aware rules. Guardrails matter most when the model is constrained; they still help when it is strong.
