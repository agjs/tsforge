...
| `TSFORGE_REVIEW_MAX_FILES` | `100` | max changed files [review](/cli/review/) covers in one run (raise for large changes; excess is queued, not dropped) |
| `TSFORGE_REVIEW_DIFF_CHARS` | `12000` | per-file diff budget before the reviewer truncates (raise to review very large single-file diffs) |
