...
tsforge also writes its **own** TTSR rules: failures it made and then fixed in a repo are mined into `.tsforge/learned-rules.json` and loaded alongside the built-ins and your custom rules. So a mistake the model keeps making gets caught mid-stream next time — automatically. See [Learning from past runs](/uplift/memory/).

## Limits

- **3 interrupts per task**, then TTSR turns off for the rest of that task

→ [Learning from past runs](/uplift/memory/) · [Big picture](/big-picture/) · [Environment variables](/reference/flags/)
