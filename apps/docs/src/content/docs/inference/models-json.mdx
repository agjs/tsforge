...
A top-level `mcpServers` block (a sibling of `active`/`models`) configures [MCP servers](/integrations/mcp/) available in **every** project, so a server you use everywhere — Linear, Sentry, Context7 — only needs to be set up once instead of copied into each repo's `tsforge.config.json`:

```json
{
  "active": "local",
  "models": { "local": { "baseUrl": "http://localhost:8000/v1", "model": "deepseek-v4-flash" } },
  "mcpServers": {
...
A project's own `tsforge.config.json` `mcpServers` entry with the same name overrides this one entirely; entries unique to either side apply together. See [MCP servers](/integrations/mcp/) for the field reference and the curated Linear/Notion/Sentry integrations.
