...
| `reasoning` | no | Provider thinking dialect: `qwen` (default), `deepseek`, `openai`, `none` — see below. |
...
| `deepseek` | `thinking: { type: "enabled"\|"disabled" }`, `reasoning_effort` | Also never sends `tool_choice: "required"` (DeepSeek's thinking mode rejects it). |
