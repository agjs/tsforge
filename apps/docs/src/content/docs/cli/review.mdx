...
When a `tsconfig.json` is present, the reviewer can follow the callers of each changed export through the type graph (`find_references` / `type_at`), so the regression check looks at concrete call sites instead of guessing.
