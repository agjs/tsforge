...
The status line above each prompt shows the last call's generation rate (`tok/s`) next to the context gauge. `/metrics` prints the running totals for the session — model calls, input/output tokens, and both the last and average output rate:
