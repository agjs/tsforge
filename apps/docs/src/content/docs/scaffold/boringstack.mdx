...
description: Stand up a new project from BoringStack, its Astro static site, or Phaser with the tsforge scaffold wizard.
---

Use this when you want a **new project from scratch**. Instead of letting the model
invent a stack, the wizard stands up [**BoringStack**](https://boringstack.xyz), a proven full-stack
...
For a Phaser 4 game, use the [Phaser archetype](/scaffold/phaser/) instead — that
clones a different repo and does not use this fullstack manifest.
...
| `phaser` | A Phaser 4 TypeScript game from [Phaser-TypeScript-AI-First-Starter](/scaffold/phaser/); no Docker, no `.env` | `bun run check` |
